...
int read_pos1() {
  return map(analogRead(POTEN_PIN), 278, 800, -90, 90) + 12;
}
...
void getAction(){
  if(Serial.available()) {
    a1 = map(Serial.read(), 0, 255, -Q_MAX, Q_MAX);
    a2 = map(Serial.read(), 0, 255, -Q_MAX, Q_MAX);
  }
    // calibrate(POS20+a1, serv1, PWM_PIN1);
    calibrate(POS30+a2, serv2, PWM_PIN2);
}
